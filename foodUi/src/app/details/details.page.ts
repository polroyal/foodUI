...
import { AfterViewInit, Component, ElementRef, OnInit, Inject, QueryList, ViewChild, ViewChildren } from '@angular/core';
...
export class DetailsPage implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.http.get('https://devdactic.fra1.digitaloceanspaces.com/foodui/1.json').subscribe((res: any) => {this.data = res;
    });

    // we set the header position for sticky slides
    ...
    // we now get all the list viewchildren when ready
